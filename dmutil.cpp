...
HRESULT DXUtil_ConvertGenericStringToWide(WCHAR* wstrDestination, const TCHAR* tstrSource, int cchDestChar = -1) {
    return DXUtil_ConvertGenericStringToWideCch(wstrDestination, tstrSource, cchDestChar);
}
...
HRESULT CMusicManager::Initialize( HWND hWnd, DWORD dwPChannels, DWORD dwDefaultPathType )
{
                                                ...
                                                dwPChannels, DMUS_AUDIOF_ALL, NULL ) ) )
    {
        ...
        {
            DXTRACE( L"Warning: No sound card found\n" );
            return hr;
        }
